...
resource "aws_codebuild_project" "this" {
  ...
  cache {
    type     = "S3"
    location = format("%s/%s", var.fdp_backend_bucket[data.aws_region.this.region], var.q.s3_cache_location)
  }
  ...
  logs_config {
    ...
    s3_logs {
      ...
      location = format("%s/%s", var.fdp_backend_bucket[data.aws_region.this.region], var.q.s3_logs_location)
    }
  }
}
