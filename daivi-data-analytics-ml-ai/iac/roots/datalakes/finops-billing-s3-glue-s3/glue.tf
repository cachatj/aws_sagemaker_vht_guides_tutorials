...
resource "aws_glue_security_configuration" "glue_security_configuration" {
  ...
  name = "glue-security-configuration-finops-billing-s3-glue-s3"
...
resource "aws_glue_crawler" "billing_crawler" {
  ...
  name          = "finops-billing-s3-glue-s3-crawler"
  ...
  depends_on = [module.billing_data_bucket, aws_s3_object.billing_data_files]
}
...
resource "aws_glue_catalog_table" "billing_hive" {
  ...
  storage_descriptor {
    ...
    location      = "s3://${module.billing_hive_bucket.bucket_name}/"
...
resource "aws_glue_data_quality_ruleset" "billing_hive_ruleset" {
  ...
  name        = "finops_billing_s3_glue_s3_hive_ruleset"
...
resource "aws_glue_data_quality_ruleset" "billing_iceberg_ruleset" {
  ...
  name        = "finops_billing_s3_glue_s3_iceberg_ruleset"
...
resource "aws_glue_job" "billing_hive_job" {
  ...
  description       = "finops-billing-s3-glue-s3-hive"
  ...
  command {
    script_location = "s3://${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-glue-scripts/finops_billing_s3_glue_s3_hive.py"
  }
...
resource "aws_glue_job" "billing_iceberg_static_job" {

  name              = "finops-billing-s3-glue-s3-iceberg-static"
  description       = "finops-billing-s3-glue-s3-iceberg-static"
  role_arn          = data.aws_iam_role.glue_role.arn
  glue_version      = "5.0"
  worker_type       = "G.1X"
  number_of_workers = 10
  ...
  command {
    script_location = "s3://${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-glue-scripts/finops_billing_s3_glue_s3_iceberg_static.py"
  }
...
resource "aws_glue_job" "billing_iceberg_dynamic_job" {

  name              = "finops-billing-s3-glue-s3-iceberg-dynamic"
  description       = "finops-billing-s3-glue-s3-iceberg-dynamic"
  role_arn          = data.aws_iam_role.glue_role.arn
  glue_version      = "5.0"
  worker_type       = "G.1X"
  number_of_workers = 10
  ...
  command {
    script_location = "s3://${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-glue-scripts/finops_billing_s3_glue_s3_iceberg_dynamic.py"
  }
...
resource "aws_glue_job" "billing_s3table_job" {

  name              = "finops-billing-s3-glue-s3table"
  description       = "finops-billing-s3-glue-s3table"
  role_arn          = data.aws_iam_role.glue_role.arn
  glue_version      = "5.0"
  worker_type       = "G.1X"
  number_of_workers = 10
  ...
  command {
    script_location = "s3://${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-glue-scripts/finops_billing_s3_glue_s3table.py"
  }

  default_arguments = {
    "--SOURCE_FILE"                       = var.BILLING_DATA_FILE
    "--NAMESPACE"                         = var.APP
    "--TABLE_BUCKET_ARN"                  = "arn:aws:s3tables:${var.AWS_PRIMARY_REGION}:${var.AWS_ACCOUNT_ID}:bucket/finops-billing-s3-glue-s3table"
...
resource "aws_glue_trigger" "billing_iceberg_static_job_trigger" {

  name     = "finops-billing-s3-glue-s3-iceberg-static-job-trigger"
  type     = "SCHEDULED"
  schedule = "cron(0 7 * * ? *)"

  actions {
    job_name = aws_glue_job.billing_iceberg_static_job.name
  }

  enabled = false
}

resource "aws_glue_trigger" "billing_s3table_job_trigger" {

  name     = "finops-billing-s3-glue-s3table-job-trigger"
  type     = "SCHEDULED"
  schedule = "cron(0 7 * * ? *)"

  actions {
    job_name = aws_glue_job.billing_s3table_job.name
  }
...
resource "aws_glue_trigger" "billing_crawler_trigger" {
  ...
  name          = "finops-billing-s3-glue-s3-crawler-trigger"
  type          = "ON_DEMAND"
...
resource "aws_glue_trigger" "billing_iceberg_dynamic_job_trigger" {

  name          = "finops-billing-s3-glue-s3-iceberg-dynamic-job-trigger"
  type          = "CONDITIONAL"
  workflow_name = aws_glue_workflow.billing_workflow.name

  actions {
    job_name = aws_glue_job.billing_iceberg_dynamic_job.name
  }
...
resource "aws_glue_workflow" "billing_workflow" {
  ...
  name        = "finops-billing-s3-glue-s3-workflow"
