...
resource "aws_sns_topic" "lambda_sns_topic" {
  ...
  name              = "finops-inventory-s3-glue-s3-lambda-topic"
  kms_master_key_id = "alias/aws/sns"
}

resource "aws_lambda_function" "inventory_workflow_trigger" {

  function_name    = "finop-inventory-s3-glue-s3-workflow-trigger"
...
resource "aws_lambda_permission" "allow_s3_invoke" {
  ...
  source_arn    = "arn:aws:s3:::${var.INVENTORY_DATA_TARGET_BUCKET_NAME}"
}
