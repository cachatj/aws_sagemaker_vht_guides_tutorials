...
module "equity_trade_data_bucket" {

  source = "../../../templates/modules/bucket"
  
  APP       = var.APP
  ENV       = var.ENV
  NAME      = "equity-trade-s3-glue-s3-data"
  USAGE     = "equity-trade"
  CMK_ARN   = data.aws_kms_key.s3_kms_key.arn
}

module "equity_trade_hive_bucket" {
...
module "equity_trade_iceberg_bucket" {
...
resource "aws_s3_object" "equity_trade_data_files" {

  for_each = fileset("${path.module}/../../../../data/equity_trades/", "*.csv")
  bucket   = module.equity_trade_data_bucket.bucket_id
  key = each.value
  source = "${path.module}/../../../../data/equity_trades/${each.value}"
  content_type = "csv"
...
resource "aws_lakeformation_resource" "hive_s3_location" {
  ...
  arn       = "arn:aws:s3:::${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-equity-trade-s3-glue-s3-hive"
...
resource "aws_lakeformation_resource" "iceberg_s3_location" {
  ...
  arn       = "arn:aws:s3:::${var.AWS_ACCOUNT_ID}-${var.APP}-${var.ENV}-equity-trade-s3-glue-s3-iceberg"
  role_arn  = data.aws_iam_role.glue_role.arn
