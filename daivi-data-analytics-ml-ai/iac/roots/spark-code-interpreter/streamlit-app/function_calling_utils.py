...
IMPORTANT: When generating PySpark code, you MUST include comprehensive logging statements throughout the code to track execution progress. This is a strict requirement. Your code MUST include:
- A properly configured logger at the beginning of the script
- Step-by-step progress logging with clear STEP markers
- Performance timing for key operations
- Data information logging (schema, counts, etc.)
- Error logging with stack traces
- Completion logging with timing information
...
CRITICAL REQUIREMENT: All generated code MUST include comprehensive logging throughout to track execution progress. Your code MUST include:
1. A properly configured logger at the beginning of the script
2. Step-by-step progress logging with clear STEP markers (e.g., "STEP 1: Loading data")
3. Performance timing for key operations
4. Data information logging (schema, counts, etc.)
5. Error logging with stack traces
6. Completion logging with timing information"""
...
- CRITICAL: Always initialize the 'output' variable at the beginning of your code: output = {}
- CRITICAL: Always ensure the 'output' variable is properly assigned before the end of the script
- CRITICAL: Always include comprehensive logging throughout your code to track execution progress and performance
...
# Set logging to warn for Spark components
spark.sparkContext.setLogLevel("WARN")
logger.info("Spark session initialized successfully")

# STEP 2: Configure Spark to use S3A FileSystem
logger.info("STEP 2: Configuring S3A filesystem")
spark._jsc.hadoopConfiguration().set("fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")

# Configure Spark with AWS credentials (uses Lambda's IAM role)
spark._jsc.hadoopConfiguration().set("fs.s3a.aws.credentials.provider", 
                                     "com.amazonaws.auth.DefaultAWSCredentialsProviderChain")
logger.info("S3A filesystem configuration completed")

try:
    # STEP 3: Read and process data
    logger.info("STEP 3: Reading data from S3")
    read_start_time = time.time()
    df = spark.read.csv("s3a://path/to/file/file.csv")
    logger.info(f"Data loaded successfully in {time.time() - read_start_time:.2f} seconds")
    
    # Log basic data information
    row_count = df.count()
    col_count = len(df.columns)
    logger.info(f"Dataset dimensions: {row_count:,} rows x {col_count} columns")
    ...
    # Your analysis code here with logging for each major operation
    # Example:
    logger.info("Calculating summary statistics")
    summary_stats = df.describe()
    logger.info(f"Analysis completed in {time.time() - analysis_start_time:.2f} seconds")
    ...
    logger.info(f"Visualizations created in {time.time() - viz_start_time:.2f} seconds")
    ...
    # Log job completion and timing
    total_time = time.time() - start_time
    logger.info(f"Spark job completed successfully in {total_time:.2f} seconds")
    ...
    with open('/tmp/output.json', 'w') as f:
        json.dump(output, f)
    logger.info("Output saved to /tmp/output.json")
"""
                                },
                                "dataset_name": {
                                    "type": "string",
...
# Add unique IDs to plotly filenames if they exist
if 'plotly-files' in output and output['plotly-files']:
    unique_id = str(uuid.uuid4())[:8]
    new_plotly_files = []
    
    for plotly_file in output['plotly-files']:
        # Get base filename and extension
        base_name, ext = os.path.splitext(plotly_file)
        # Create new filename with unique ID
        new_filename = f"{base_name}_{unique_id}{ext}"
        
        # Rename the file
        if os.path.exists(f'/tmp/{plotly_file}'):
            os.rename(f'/tmp/{plotly_file}', f'/tmp/{new_filename}')
            new_plotly_files.append(new_filename)
    
    # Update the output with new filenames
    output['plotly-files'] = new_plotly_files
